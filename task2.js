...
function task21(arr) {
    ...
    const opsByMonth = arr.reduce((acc, { year, month }) => {
    ...
    const monthlyOpsArray = Object.entries(opsByMonth).map(([key, opsCount]) => {
        const [year, month] = key.split('-').map(Number);
        return { year, month, opsCount };
    });


    monthlyOpsArray.sort((a, b) => b.opsCount - a.opsCount);


    return monthlyOpsArray.slice(0, 3);
}

// console.log(task21(ops))
// ================================================================

//     { "year": 2019, "month": 1, "day": 1, "type": "replenishment", "amount": 79817 },

// console.log(task22(2019, 2, ops))
...
function task22(year, month, arr) {
    ...
    const filteredReplen = arr.filter(item => item.month === month && item.year === year && item.type === "replenishment");
    const filteredPay = arr.filter(item => item.month === month && item.year === year && item.type === "payment");
    const filteredWithdrawal = arr.filter(item => item.month === month && item.year === year && item.type === "withdrawal");

    const sumReplenishment = filteredReplen.reduce((acc, curr) => acc + curr.amount, 0);
    const sumPayment = filteredPay.reduce((acc, curr) => acc + curr.amount, 0);
    const sumWithdrawal = filteredWithdrawal.reduce((acc, curr) => acc + curr.amount, 0);
    const totalDays = filteredReplen.length + filteredPay.length;
    const formattedDate = formatDate(year, month + 1, totalDays);
    const monthBalance = sumReplenishment - sumPayment;
    const withdrawalRate = sumPayment / sumReplenishment;
...
function task23(arr) {
    ...
    items = items.map((item) => {
        let totalBalance = accumulation + item.monthBalance;
        accumulation = totalBalance;
